...
namespace BAstar_planner
{
  ...
  //函数名:findPath
  //输入:地图布局，起始和目标单元格，以及一个布尔值，表示我们是否使用break ties 
  //输出:最佳路径
  //描述:用于生成机器人的自由路径
  /*********************************************************************************/
  vector<int> BAstarPlannerROS::findPath(int startCell, int goalCell, float g_score[])
  {
    ...
    //calculate g_score and f_score of the start positions  
    //计算起始位置的g score和f_score
...
S
  ...
  /*******************************************************************************/
  //Function Name: calculateHCost// 计算启发式代价
  ...
  {    
    ...
    //distance = sqrt(pow(x1,x2)+pow(y1,y2));       //Euclidean Distance  欧几里得距离
    //distance = min(abs(x1-x2),abs(y1-y2))  *sqrt(2)  + max(abs(x1-x2),abs(y1-y2)) - min(abs(x1-x2),abs(y1-y2));   //Diagonal Distance 对角线的距离
    
    //h = w * distance;

    // Calculate the line equation coefficients (a, b, c) for the line connecting (x1, y1) and (x2, y2)
    double a = y2 - y1;
    double b = x1 - x2;
    double c = x2 * y1 - x1 * y2;

    // Calculate the perpendicular distance from point (x0, y0) to the line ax + by + c = 0
    double lineDistance = abs(a * x2 + b * y2 + c) / sqrt(a * a + b * b);

    // Calculate the squared Euclidean distance from point (x0, y0) to the goal point (x2, y2)
    int goalDistanceSquared = (x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1);

    // Calculate the heuristic value based on the line distance and the square root of the goal distance squared
    h = lineDistance + sqrt(goalDistanceSquared);
